...
import { DEFAULT_POLICY, STREAK_POLICY, stopReason, type Policy, type Position } from "./policy.js";
...
export class Roller {
  ...
  open(asset: string, up: boolean, stakeHuman: number, decimals: number, policy: Policy): Position {
    const stake = BigInt(Math.round(stakeHuman * 10 ** decimals));
    const p: Position = {
    ...
    save(this.positions);
    log(`opened #${p.id}  ${asset} ${up ? "UP" : "DOWN"}  stake ${fmt(stake, decimals)}`);
    return p;
  }
  ...
  async run(): Promise<void> {
    ...
    for (;;) {
      ...
      if (!this.positions.some((p) => p.active)) {
  ...
  private async harvest(): Promise<void> {
    ...
    for (const p of exposed) {
      ...
      const stakeIn = p.stake;
      p.losses = payout < stakeIn ? p.losses + 1 : 0;
      p.stake = p.policy.compound ? payout : payout > 0n ? p.principal : 0n;
      p.rolls += 1;
      p.history.push({
        marketId: p.marketId!,
        stakeIn: stakeIn.toString(),
        stakeOut: payout.toString(),
        won,
      });
      ...
      log(
        `#${p.id} roll ${p.rolls}  ${voided ? "VOID" : won ? "WON " : "LOST"}  ` +
          `${fmt(stakeIn, d)} → ${fmt(payout, d)}  (streak ${p.rolls - p.losses}/${p.rolls})`,
      );
      ...
      if (stop) {
        p.active = false;
        log(`#${p.id} CLOSED (${stop}) — ${fmt(p.principal, d)} in, ${fmt(p.stake, d)} out`);
      }
    }
  }
  ...
  private async enter(windows: Window[]): Promise<void> {
    ...
    for (const p of pending) {
      ...
      const quote = quoteBinaryStakeOverBook(book, side, p.stake, oneCollateral, {
        tickSize: w.tickSize,
        lotSize: w.lotSize,
      ...
      log(
        `#${p.id} entered ${w.symbolUp.split("/")[0]} ${p.up ? "UP" : "DOWN"} ` +
          `@ ${priceHuman.toFixed(3)}  size ${fmt(quote.quantity, w.decimals)}  ` +
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const roller = new Roller(ex, account, { live, cadenceSec });
