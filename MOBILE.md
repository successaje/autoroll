...
The Mac runs the keeper; the phone uses Safari, Chrome, or the installed PWA. Tap Connect MetaMask to establish a MetaMask Connect session. MetaMask opens for connection and transaction approvals; return to the original browser/PWA to follow the position. Each browser/PWA context authorizes its own session.
